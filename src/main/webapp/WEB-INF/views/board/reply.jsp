...
<script type="text/javascript">
	...
	$("#replyInsert").click(function(){
		...
		if(!chkData("#r_name", "이름을")) return;
			...
			// 클릭시 유효성 체크를 한뒤 else로 이상없을시 비동기 실행.
	...
	function listAll(b_num){
		...
		$.getJSON(url, function(data){
			...
			alert("댓글 목록을 불러오는데 실패하였습니다. <br />잠시후에 다시 시도하십시오");
		});
	}
	...
	function addNewItem(r_num, r_name, r_content, r_date){
		// 새로운 글이 추가될 li태그 객체, 속성과 클래스명 추가
		var new_li = $("<li>");
			...
			<form id="comment_form">
				<div>
					<label for="r_name">작성자</label> <input type="text" name="r_name"
						...
						type="password" id="r_pwd" name="r_pwd" /> <input type="button"
